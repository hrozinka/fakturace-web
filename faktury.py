...
st.markdown("""
    <style>
    .stApp { background-color: #0f172a !important; color: #f8fafc !important; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
    h1, h2, h3, h4, h5, h6, p, label, span, div, li { color: #f8fafc !important; }
    .stTextInput input, .stNumberInput input, .stTextArea textarea, .stDateInput input { background-color: #1e293b !important; border: 1px solid #334155 !important; color: #fff !important; border-radius: 12px !important; padding: 12px !important; }
    .stSelectbox div[data-baseweb="select"] { background-color: #1e293b !important; border: 1px solid #334155 !important; color: #ffffff !important; border-radius: 12px !important; }
    ul[data-baseweb="menu"] { background-color: #0f172a !important; border: 1px solid #334155 !important; padding: 5px !important; }
    li[data-baseweb="option"] { background-color: #0f172a !important; color: #ffffff !important; padding: 10px !important; }
    li[data-baseweb="option"] div { color: #ffffff !important; }
    li[data-baseweb="option"][aria-selected="true"], li[data-baseweb="option"]:hover { background-color: #fbbf24 !important; cursor: pointer; }
    li[data-baseweb="option"][aria-selected="true"] div, li[data-baseweb="option"]:hover div { color: #0f172a !important; font-weight: bold !important; }
    ...
    section[data-testid="stSidebar"] .stRadio label { background-color: #1e293b !important; padding: 15px !important; border-radius: 10px !important; border: 1px solid #334155 !important; width: 100% !important; margin-bottom: 5px; }
    section[data-testid="stSidebar"] .stRadio label[data-checked="true"] { background: linear-gradient(135deg, #fbbf24 0%, #d97706 100%) !important; color: #0f172a !important; }
    section[data-testid="stSidebar"] .stRadio label[data-checked="true"] p { color: #0f172a !important; }
    .stButton > button, [data-testid="stDownloadButton"] > button { background-color: #334155 !important; color: #ffffff !important; border-radius: 10px !important; height: 50px; font-weight: 600; border: 1px solid #475569 !important; width: 100%; }
    ...
    .stat-box, .mini-stat-box { background: #1e293b; border-radius: 12px; padding: 15px; text-align: center; border: 1px solid #334155; }
    .text-green span { color: #34d399 !important; } 
    .text-red span { color: #f87171 !important; } 
    .text-gold span { color: #fbbf24 !important; }
    ...
    .app-logo { font-size: 60px; margin-bottom: 0px; text-align: center; display: block; }
            ...
            with st.form("forgot"):
                ...
                if st.form_submit_button("Resetovat heslo", use_container_width=True):
                    ...
                    if usr:
                        ...
                        else: st.error("Chyba při odesílání emailu. Zkontrolujte nastavení SMTP.")
...
if role == 'admin':
    ...
    c1.metric("Počet uživatelů", u_count); c2.metric("Celkový obrat", f"{t_rev:,.0f} Kč"); c3.metric("Obrat / Uživatel", f"{avg_u:,.0f} Kč"); c4.metric("Průměrná faktura", f"{avg_f:,.0f} Kč")
    st.divider()
    ...
    with tabs[0]:
        ...
        for u in run_query("SELECT * FROM users WHERE role!='admin' ORDER BY id DESC"):
            ...
            with st.expander(f"{u['username']} ({u['email']}) | {status_badge}"):
                c1, c2 = st.columns(2); c1.write(f"**Jméno:** {u['full_name']}"); c1.write(f"**Tel:** {u['phone']}"); c1.write(f"**Vytvořeno:** {format_date(u['created_at'])}")
    ...
    with tabs[2]:
        ...
        with st.form("wm"):
            ...
            if st.form_submit_button("Uložit šablonu"): run_command("INSERT INTO email_templates (name, subject, body) VALUES ('welcome', ?, ?) ON CONFLICT (name) DO UPDATE SET subject = EXCLUDED.subject, body = EXCLUDED.body", (ws, wb)); st.success("OK")
        st.divider(); st.subheader("Hromadné odeslání zprávy")
        with st.form("mm"):
...
# USER
    ...
    if "Faktury" in menu:
        ...
        pausal_opt = c_pausal.selectbox("Typ činnosti (Paušální výdaje)", ["80% - Řemeslné živnosti, zemědělství", "60% - Ostatní živnosti (nejčastější)", "40% - Svobodná povolání, autorská práva", "30% - Pronájem majetku"], index=1)
        ...
        with st.expander("➕ Přidat"):
            ...
            with st.form("fc"):
                ...
                if st.form_submit_button("Uložit"): run_command("INSERT INTO klienti (user_id, jmeno, adresa, ico, dic, poznamka) VALUES (?,?,?,?,?,?)", (uid,j,a,i,d,p)); reset_forms(); get_cached_pdf.clear(); get_cached_isdoc.clear(); st.rerun()
        ...
        for k in run_query("SELECT * FROM kategorie WHERE user_id=?", (uid,)):
            with st.expander(k['nazev']):
                if k['logo_blob']: st.image(bytes(k['logo_blob']), width=100)
        ...
        with st.expander("🔑 Licence", expanded=True):
            ...
            if not valid:
                st.markdown("""
                <div style="background-color: #334155; padding: 15px; border-radius: 10px; border: 1px solid #475569; margin-bottom: 20px;">
                    <h3 style="color: #fbbf24 !important;">🚀 Získejte MojeFaktury PRO</h3>
                    <ul style="color: #cbd5e1;">
                        <li>Neomezený počet faktur</li>
                        <li>Export pro účetní (ISDOC)</li>
                        <li>Daňová kalkulačka</li>
                        <li>Vlastní logo a barvy</li>
                        <li>Cloud záloha dat</li>
                    </ul>
                    <p><b>Cena: 990 Kč / rok</b></p>
                    <p>Pro aktivaci napište na: <a href="mailto:jsem@michalkochtik.cz" style="color:#fbbf24;">jsem@michalkochtik.cz</a></p>
                </div>
                """, unsafe_allow_html=True)
                k = st.text_input("Máte klíč? Zadejte ho zde:")
                ...
                st.info(f"✅ Máte aktivní PRO licenci do: **{format_date(exp)}**")
        ...
        with st.expander("🏢 Moje Firma"):
            with st.form("setf"):
                n=st.text_input("Název", c.get('nazev', full_name_display)); a=st.text_area("Adresa", c.get('adresa','')); i=st.text_input("IČO", c.get('ico','')); d=st.text_input("DIČ", c.get('dic','')); b=st.text_input("Banka", c.get('banka','')); u=st.text_input("Účet", c.get('ucet','')); ib=st.text_input("IBAN", c.get('iban',''))
                if st.form_submit_button("Uložit"):
        ...
        with st.expander(f"🔔 Upozornění {'(PRO)' if not is_pro else ''}"):
            if not is_pro: st.warning("🔒 Pouze pro PRO verzi.")
                ...
                st.markdown("### SMTP Server"); preset = st.selectbox("Rychlé nastavení", ["-- Vyberte --", "Seznam.cz", "Gmail", "Vlastní"])
                ...
                s_server = st.text_input("Server", value=d_srv); c3, c4 = st.columns(2); s_port = c3.number_input("Port", value=d_prt); s_user = c4.text_input("Login", value=c.get('smtp_email', '')); s_pass = st.text_input("Heslo", value=c.get('smtp_password', ''), type="password")
                c5, c6 = st.columns(2)
                ...
                if c6.button("📨 Test"): 
                    if send_email_custom(n_email, "Test", "Testovací zpráva"): st.success("OK")
                    else: st.error("Chyba")

        if is_pro:
            with st.expander("📦 Export ISDOC"):
                c1, c2 = st.columns(2); d_start = c1.date_input("Od", date.today().replace(day=1)); d_end = c2.date_input("Do", date.today())
                if st.button("Stáhnout ZIP"):
                    ...
                    if invs:
                        ...
                        st.download_button("Stáhnout", buf.getvalue(), "export.zip", "application/zip")
                    else: st.warning("Žádná data")

            with st.expander("💾 Zálohování"):
                st.download_button("Export JSON", get_export_data(uid), "zaloha.json", "application/json")
                if st.button("Odeslat na Email"):
                     if send_email_custom(c.get('notify_email'), "Záloha", "JSON v příloze", get_export_data(uid), "zaloha.json"): st.success("Odesláno")
                     else: st.error("Chyba")
                ...
                if upl and st.button("Obnovit"):
                    try:
                        ...
                        get_cached_pdf.clear(); get_cached_isdoc.clear(); st.success("Import hotov"); st.rerun()
