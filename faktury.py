...
# Na webu nemůžeme používat složku Dokumenty uživatele, použijeme aktuální složku
APP_DIR = "." 
...
def generate_pdf(faktura_id):
    ...
    class PDF(FPDF):
        def header(self):
            font_path = 'arial.ttf' # Uprava cesty fontu pro Linux/Web
            self.font_ok = False
            # Na webu neni Windows font, pouzijeme default nebo pribaleny
    ...
    try:
        ...
        def stxt(t):
            t = str(t) if t else ""
            # Pokud neni font, odstranime hacky
            return t if getattr(pdf, 'font_ok', False) else remove_accents(t)
        
        fbody = 'ArialCS' if getattr(pdf, 'font_ok', False) else 'Arial'
        pdf.set_font(fbody, '', 10)
        ...
        pdf.set_font(fbody, '', 12); pdf.cell(95, 5, stxt(moje.get('nazev','')), 0, 1)
        pdf.set_font(fbody, '', 10); pdf.multi_cell(95, 5, stxt(f"{moje.get('adresa','')}\nIČ: {moje.get('ico','')}\nDIČ: {moje.get('dic','')}\n{moje.get('email','')}"))
        pdf.set_xy(105, y); pdf.set_font(fbody, '', 12); pdf.cell(95, 5, stxt(data['k_jmeno']), 0, 1)
        pdf.set_xy(105, pdf.get_y()); pdf.set_font(fbody, '', 10); pdf.multi_cell(95, 5, stxt(f"{data['k_adresa']}\nIČ: {data['k_ico']}\nDIČ: {data['k_dic']}"))
        
        pdf.ln(10); pdf.set_fill_color(r, g, b); pdf.rect(10, pdf.get_y(), 190, 2, 'F'); pdf.ln(5)
        pdf.set_font(fbody, '', 14); pdf.cell(100, 8, stxt(f"Faktura č.: {data['cislo_full']}"), 0, 1)
        pdf.set_font(fbody, '', 10); y_d = pdf.get_y()
        ...
        pdf.ln(15)
        if data['uvodni_text']: pdf.set_font(fbody, '', 10); pdf.multi_cell(190, 5, stxt(data['uvodni_text']), 0, 'L'); pdf.ln(5)
        ...
        style_b = 'B' if getattr(pdf, 'font_ok', False) else ''
        pdf.set_font(fbody, style_b, 14); pdf.set_text_color(0, 0, 0)
        pdf.cell(40, 6, stxt("CELKEM:"), 0, 0, 'L'); pdf.cell(45, 6, stxt(f"{data['castka_celkem']:,.2f} Kč").replace(",", " "), 0, 1, 'R')
        
        pdf.ln(25); pdf.set_font(fbody, '', 10); pdf.set_text_color(50, 50, 50); pdf.set_x(120)
    ...
    try:
        ...
        if key in valid_keys_db:
            ...
            if not info.get("active", True):
                return False, "Licence byla zablokována / deaktivována.", None, None
            
            exp_str = info.get("exp", "2099-12-31")
            try:
                exp_date = datetime.strptime(exp_str, "%Y-%m-%d").date()
                if date.today() > exp_date:
                    return False, f"Licence vypršela dne {format_date(exp_date)}.", None, None
            except: pass

            owner_name = info.get("note", "Uživatel")
            return True, "Platná licence", owner_name, exp_str
...
def verify_app_access():
    ...
    last_check = settings.get('last_license_check')
    ...
    if last_check:
        try:
            ld = datetime.strptime(last_check, "%Y-%m-%d").date()
            if (date.today() - ld).days < 3: needs_check = False
        except: pass
...
if menu == "Nastavení":
    ...
    with st.expander("💾 Záloha"):
        c1,c2=st.columns(2)
        c1.download_button("Stáhnout JSON", export_data_to_json(), f"backup_{date.today()}.json", "application/json")
        upl = c2.file_uploader("Nahrát JSON", type="json")
        if upl and c2.button("Obnovit"):
            if import_data_from_json(upl): st.success("Hotovo"); st.rerun()
